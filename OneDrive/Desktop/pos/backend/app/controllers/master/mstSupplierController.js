//INCLUDES
const { Supplier } = require("../../models/master/supplierModels");
...
exports.getAll = async (req, res, next) => {
  try {
    const query=`
      ...
      FROM [MstSupplier] 
      INNER JOIN MstUser AS EntryUser ON EntryUser.Id = [MstSupplier].EntryUserId
      INNER JOIN MstUser AS UpdateUser ON UpdateUser.Id = [MstSupplier].UpdateUserId
    `;
...
****************************************************************/
exports.getAllSupplierAccount = async (req, res, next) => {
  try {
    let query = `SELECT 
                  MstAccount.Id, 
                  MstAccount.Account, 
    ...
    return res.status(500).json({ error: "Controller Error[getAllSupplierAccount]", message: error.message }); //SOMETHING WENT WRONG ABOVE, INSIDE THE TRY{ code here }
  }
};//END HERE
//***************************************************************
...
****************************************************************/
exports.getAllSupplierTerm = async (req, res, next) => {
  try {
    let query = `SELECT 
                  MstTerm.Id, 
                  MstTerm.Term, 
    ...
    return res.status(500).json({ error: "Controller Error[getAllSupplierTerm]", message: error.message }); //SOMETHING WENT WRONG ABOVE, INSIDE THE TRY{ code here }
  }
};//END HERE
//***************************************************************
...
exports.find = async (req, res) => {
  try {
    ...
    if (dataId) {
      const data = await Models.find(dataId, '[MstSupplier]'); // FIND THE ITEM TO CONFIRM THE ITEM EXISTS
...
exports.delete = async (req, res) => {
  try {
    ...
    const result = await Models.find(dataId, '[MstSupplier]'); // FIND THE ITEM TO CONFIRM THE ITEM EXISTS
    if (!result)  return res.status(404).json({ message: "Data not found" });  //IF ITEM WASN'T FOUND THEN SEND THE ERROR MESSAGE
    
    const result2 = await Models.delete(dataId, '[MstSupplier]'); //IF THE ITEM IS FOUND, THEN DELETE THE ITEM
...
exports.add= async (req, res) => {
  try {
    const data = req.body;
    const requiredFields = [ 'Supplier', 'Address', 'TelephoneNumber', 'CellphoneNumber', 'FaxNumber','TermId', 'TIN',  'AccountId',    'EntryUserId', 'EntryDateTime','UpdateUserId', 'UpdateDateTime' ];
    const missingFields = [];
    ...
    const checkData = await Models.findWithQuery(`SELECT Id FROM [MstSupplier] WHERE Supplier='${data.Supplier}' `);
    if (checkData) {return res.status(400).json({ message: 'Data already exists.' });}

    const result = await Supplier.add(data);
...
exports.update= async (req, res) => {
  try {
    ...
    const selectedData = await Models.find(dataId, '[MstSupplier]');  // FIND THE ITEM TO CONFIRM THE DATA EXISTS
    ...
    const checkData = await Models.findWithQuery(`SELECT Id FROM [MstSupplier] WHERE Id<>'${dataId}' AND Supplier='${data.Supplier}' `);
    if (checkData) {return res.status(400).json({ message: 'Data already exists.' });}

    const result = await Supplier.update(dataId, data);  //IF ALL REQUIRED FIELDS ARE PRESENT THEN UPDATE THE DATA
    if (result) return res.status(201).send({ message: 'Item updated successfully.' }); //UPDATING DATA IS SUCCESFULL
...
exports.lock= async (req, res) => {
  try {
    const dataId = req.params.Id; // Assuming req.params.Id contains the data's ID
    const selectedData = await Models.find(dataId, '[MstSupplier]'); // FIND THE DATA TO CONFIRM THE DATA EXISTS
    if (!selectedData) return res.status(404).json({ message: "Data not found" });
    
    const result = await Models.lock(dataId, '[MstSupplier]'); //IF ALL REQUIRED FIELDS ARE PRESENT THEN UPDATE THE DATA
...
exports.unlock= async (req, res) => {
  try {
    const dataId = req.params.Id; // Assuming req.params.Id contains the data's ID
    const selectedData = await Models.find(dataId, '[MstSupplier]'); // FIND THE ITEM TO CONFIRM THE ITEM EXISTS
    if (!selectedData) return res.status(404).json({ message: "Data not found" });
    
    const result = await Models.unlock(dataId, '[MstSupplier]'); //IF ALL REQUIRED FIELDS ARE PRESENT THEN UPDATE THE DATA
...
exports.get = async (req, res) => {
  try {
    const dataId = req.params.Id; // Assuming req.params.Id contains the item's ID
    if (dataId) {
      const data = await Models.get(dataId, '[MstSupplier]'); // FIND THE ITEM TO CONFIRM THE ITEM EXISTS
      if (!data) return res.status(404).json({ message: "Data not found" });
