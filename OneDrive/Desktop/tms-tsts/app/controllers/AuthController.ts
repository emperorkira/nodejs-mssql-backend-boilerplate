...
export async function loginUser(req:any, res:any) {
  try {
    const { table } = req.params, { Username, Password } = req.body, { error } = loginSchema.validate({ Username, Password })
    ...
    if (!user) res.status(401).json({ Login: false, message: Error.invalidUsername })
    const decryptedPassword = await AuthModel.decryptPassword(user.Password)
    if (Password !== decryptedPassword) res.status(401).json({ Login: false, message: Error.invalidPass })
    const accessToken = await AuthModel.generateToken(user)
    const refreshToken = await AuthModel.generateRefreshToken(user)
    return ( res.header("Authorization", accessToken).header("Refresh-Token", refreshToken).json({ Login: true, message: Success.login, user, accessToken, refreshToken }))
  } catch (error) {
...
export async function addNewUser(req:any, res:any) {
    const { table }:any = req.params, recordData:any = req.body, id = null
...
export async function decrypt(req:any, res:any) {
  try {
    const { Id } = req.params
    const getResponse = await UserModel.get("User", "Id", Id)
    const decryptPass = await AuthModel.decryptPassword(getResponse.Password)
