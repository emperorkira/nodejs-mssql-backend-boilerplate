...
export function formatResolutionTime(seconds:any) {
...
export function formatMonthYear(year:any, month:any) {
...
export const ReportModel = {
     ...
     *****************************************************************/
    async getTicketCount(table:any) {
        const query = `SELECT COUNT(*) AS totalTickets,
            ...
            SUM(CASE WHEN Status = 1 THEN 1 ELSE 0 END) AS SolvedCount FROM [${table}]`
        const result = await sql.query(query)
        const totalCount =
        result.recordset.length > 0 ? result.recordset[0].totalCount : 0
        const records = result.recordset.map((record:any) => { record.AVGResolutionTime = formatResolutionTime( record.AVGResolutionTimeInSeconds )
     ...
     * FUNCTION NAME        : getTicketCount
     *****************************************************************/
    async getTicketCountById(table:any, id:any) {
        const query = ` SELECT COUNT(*) AS totalTickets, AVG(CASE WHEN DoneDate IS NOT NULL THEN DATEDIFF(second, DoneDate, DateCreated) END) AS AVGResolutionTimeInSeconds,
            SUM(CASE WHEN Status = 0 THEN 1 ELSE 0 END) AS OngoingCount, SUM(CASE WHEN Status = 1 THEN 1 ELSE 0 END) AS SolvedCount FROM [${table}] WHERE AssignedBy = '${id}'`
        const result = await sql.query(query)
     ...
     *****************************************************************/
    async getMonthlyTicketCounts(table:any) {
        const query = `SELECT YEAR(DoneDate) AS Year, MONTH(DoneDate) AS Month, COUNT(*) AS TotalTickets, SUM(CASE WHEN Status = 1 THEN 1 ELSE 0 END) AS SolvedCount, AVG(DATEDIFF(MINUTE, DateCreated, DoneDate)) AS AvgResolutionTimeMinutes FROM [${table}]
        WHERE DoneDate >= DATEADD(MONTH, -6, GETDATE()) AND (DoneDate IS NOT NULL OR Status = 0)
        GROUP BY YEAR(DoneDate), MONTH(DoneDate) ORDER BY Year DESC, Month DESC`
        const result = await sql.query(query)
        const monthlyCounts = result.recordset.map((record:any) => {
            const avgResolutionMinutes = record.AvgResolutionTimeMinutes
            const avgResolutionDays = Math.floor(avgResolutionMinutes / (24 * 60))
            const avgResolutionHours = Math.floor((avgResolutionMinutes % (24 * 60)) / 60 )
    ...
    async getMonthlyTicketCountsById(table, id) {
        const query = `SELECT YEAR(DoneDate) AS Year, MONTH(DoneDate) AS Month, COUNT(*) AS TotalTickets, SUM(CASE WHEN Status = 1 THEN 1 ELSE 0 END) AS SolvedCount, AVG(DATEDIFF(MINUTE, DateCreated, DoneDate)) AS AvgResolutionTimeMinutes
            FROM [${table}]  WHERE DoneDate >= DATEADD(MONTH, -6, GETDATE()) AND (DoneDate IS NOT NULL OR Status = 0) AND AssignedBy = '${id}' GROUP BY YEAR(DoneDate), MONTH(DoneDate) ORDER BY Year DESC, Month DESC `
        const result = await sql.query(query)
        const monthlyCounts = result.recordset.map((record:any) => {
            const avgResolutionMinutes = record.AvgResolutionTimeMinutes
            const avgResolutionDays = Math.floor(avgResolutionMinutes / (24 * 60))
     ...
     *****************************************************************/
    async getAverageRate(table:any) {
        const query = `SELECT  SUM(SatisfactoryRate) AS TotalSatisfactoryRate,  COUNT(*) AS TotalRecords FROM [${table}]`
        const result = await sql.query(query)
        const totalSatisfactoryRate = result.recordset.length > 0 ? result.recordset[0].TotalSatisfactoryRate : 0
        const totalRecords = result.recordset.length > 0 ? result.recordset[0].TotalRecords : 0
        const averageRate =  totalRecords > 0 ? totalSatisfactoryRate / totalRecords : 0
