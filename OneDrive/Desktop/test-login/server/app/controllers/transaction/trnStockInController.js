...
exports.getAll = async (req, res, next) => {
  try {
    ...
    // access right
    const data = await o6D646C73.f67746C6C('[TrnStockOut]');
    return res.status(200).json(data);
  } catch (error) {
...
exports.get = async (req, res) => {
  try {
    ...
    const data = await o6D646C73.f676574514644('SELECT * FROM [TrnStockOut] WHERE Id = @Id', ['Id'], [o73716C.Int], [c4964]);
...
exports.find = async (req, res) => {
  try {
    ...
    if (!c4964) return res.status(400).json({ message: 'Id is null or undefined' });
    const data = await o6D646C73.f666E64(c4964, '[TrnStockOut]');
    if (!data) return res.status(404).json({ message: 'Data not found.' });
