...
/****************************************************************
  ...
  PURPOSE              : to get all period data in period table
...
exports.k67746C6C = async (req, res) => {
  try {
    ...
    return res.status(200).json(await o6D646C73.f67746C6C(o02x747874.t707264)); 
  } catch (error) {
...
/****************************************************************
  STATUS               :  WORKING
  DATE CREATED/UPDATED : February 03, 2024
  PURPOSE              : to get specific period data by Id
  PROGRAMMER           : MARK DINGLASA
...
exports.k676574 = async (req, res) => {
  try {
    ...
    const d72736C74 = await o6D646C73.f676574(c4964, o02x747874.t707264);
...
/****************************************************************
  STATUS               :  WORKING
  DATE CREATED/UPDATED : February 03, 2024
  PURPOSE              : to find specific period by Id
  PROGRAMMER           : MARK DINGLASA
...
exports.k666E64 = async (req, res) => {
  try {
    ...
    if (!(await o617468.k6341637352(c7573724964, 12, 'CanView'))) return res.status(203).json({ message: o01x747874.e00x11 });
    if (!c4964) return res.status(203).json({ message: o01x747874.e00x06, category:'error'});
    if (!(await o6D646C73.f666E64(c4964, o02x747874.t707264))) return res.status(203).json({ message: o01x747874.e00x03, category:'error' });
...
/****************************************************************
  STATUS               :  WORKING
  DATE CREATED/UPDATED : February 03, 2024
  PURPOSE              : to delete specific period by Id
  PROGRAMMER           : MARK DINGLASA
...
exports.k646C74 = async (req, res) => {
  try {
    ...
    if (!c4964) return res.status(203).json({ message: o01x747874.e00x06, category:'error' });
    if (!(await o6D646C73.f666E64(c4964, o02x747874.t707264)))  return res.status(203).json({ message: o01x747874.e00x03, category:'error'});
    if ((await o6D646C73.f666E64514644(o04x747874.q0px01, ['c4964'], [o73716C.Int], [c4964])) || c4964 <= 10) return res.status(203).json({ message: o01x747874.e00x02, category:'error'});
    if (!(await o6D646C73.f646C74(c4964, o02x747874.t707264))) return res.status(203).json({ message: o01x747874.e00x01, category:'error'});
...
/****************************************************************
  STATUS               :  WORKING
  DATE CREATED/UPDATED : February 03, 2024
  PURPOSE              : to add new data in period table
  PROGRAMMER           : MARK DINGLASA
  FUNCTION NAME        : add(data)
****************************************************************/
exports.k616464 = async (req, res) => {
  try {
    const o01x747874 = await f72646C6E(e747874), o02x747874 = await f72646C6E(t747874), o03x747874 = await f72646C6E(s747874), o04x747874 = await f72646C6E(q747874), c64617461 = await req.body, ar72717264666C64 = [ 'Period' ], c7573724964 =  ((res.locals.user && res.locals.user.length > 0)? parseInt(await res.locals.user[0].Id):0);
    ...
    if (await o6D646C73.f666E64514644(o04x747874.q0px02, ['Period'], [o73716C.NVarChar(50)], [c64617461.Period])) {return res.status(203).json({ message: o01x747874.e00x04, category:'error'});}
    if (!/^[0-9\s]+$/.test(c64617461.Period)) return res.status(203).json({ message: o01x747874.e01x707264, category: 'error' });
    if (!(await o6D646C73.f616464514644(o02x747874.t707264, ['Period'], [o73716C.NVarChar(50)], [c64617461.Period]))) return res.status(500).send({ message: o01x747874.e00x03, category:'error'});
...
/****************************************************************
  STATUS               :  WORKING
  DATE CREATED/UPDATED : February 03, 2024
  PURPOSE              : to update specific data by Id
  PROGRAMMER           : MARK DINGLASA
  FUNCTION NAME        : update(id, data)
****************************************************************/
exports.k75706474 = async (req, res) => {
  try {
    const o01x747874 = await f72646C6E(e747874), o02x747874 = await f72646C6E(t747874), o03x747874 = await f72646C6E(s747874), o04x747874 = await f72646C6E(q747874), c4964 = parseInt(req.params.Id, 10), c64617461 = await req.body,  ar72717264666C64 = [ 'Period' ], c7573724964 =  ((res.locals.user && res.locals.user.length > 0)? parseInt(await res.locals.user[0].Id):0);
    const c6D73736E67666C = ar72717264666C64.filter(c72717264666C64 => c64617461[c72717264666C64] === undefined || c64617461[c72717264666C64] === null), c6D7376E67666C644E = c6D73736E67666C.join(', ');
    if (!c7573724964) return res.status(203).json({ message: o01x747874.err04x000002, category: 'error' });
    ...
    if (!(await o6D646C73.f666E64(c4964,o02x747874.t707264))) return res.status(203).json({ message: o01x747874.e00x03, category:'error'});
    if (c6D73736E67666C.length > 0) return res.status(203).send({ message: `Required fields [${c6D7376E67666C644E}] are missing.` , category:'error'});
    if (await o6D646C73.f666E64514644(o04x747874.q0px02, ['Id','Period'], [o73716C.Int, o73716C.NVarChar(50)], [c4964, c64617461.Period])) return res.status(203).json({ message: o01x747874.e00x03, category:'error'});
    if (!/^[0-9\s]+$/.test(c64617461.Period)) return res.status(203).json({ message: o01x747874.e01x707264, category: 'error' });
    if (!(await o6D646C73.f75706474514644(c4964, o02x747874.t707264, ['Period'], [o73716C.NVarChar(50)], [c64617461.Period]))) return res.status(203).send({ message: o01x747874.e00x01, category:'error' });
