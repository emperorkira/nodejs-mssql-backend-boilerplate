 ...
 *****************************************************************/
export async function addMultipleRecord(req, res) {
...
export async function updateRecord(req:any, res:any) {
    ...
    try {
        if (checkRecord) res.status(500).json({ message: Error.alreadyExist })
...
export async function deleteAllRecord(req:any, res:any) {
  try {
    const { table } = req.params
    const records = await UserModel.getAll(table), nonDefaultRecords = records.filter( (record) => !isDefaultRecord(table, record.Id))
