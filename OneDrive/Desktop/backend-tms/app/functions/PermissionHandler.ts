...
import { crudPermissionsMap } from '../models'
...
export async function getRolePermissions(roleId) {
    try {
        ...
        const existingRole = acc.find((role:any) => role.Id === curr.Id)
        if (!existingRole) {
            acc.push({
            Id: curr.Id,
            Code: curr.Code,
 ...
 *****************************************************************/
export async function getRoleAccessRights(roleId) {
  try {
    const request = new sql.Request()
    request.input("roleId", sql.Int, roleId)
 ...
 *****************************************************************/
export async function getAccessRightPermission(roleId) {
  try {
    const request = new sql.Request()
    request.input("RoleId", sql.Int, roleId)
