...
export const NotifModel = {
  async getDeviceToken(userId:any) {
   ...
   *****************************************************************/
  async getNotify(table:any, userId:any) {
    const request = new sql.Request()
  ...
  async addToken(userId:any, token:any, createdBy:any, updatedBy:any) {
    const request = new sql.Request()
  ...
  async removeToken(userId:any, tokenToRemove:any, updatedBy:any) {
    const request = new sql.Request()
    const existingTokens = await this.getDeviceToken(userId)
    const dateToday = new Date()
    dateToday.setUTCHours(dateToday.getUTCHours() + 8)
