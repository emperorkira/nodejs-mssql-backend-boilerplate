...
export const UserModel = {
    ...
    async create(table:any, recordData:any) {
        ...
        const generator = tableToRecordDataProperty[table] ? generateCodeWithDate : generateCode
    ...
    async createNoCode(table:any, recordData:any) {
        ...
        .map((value) => {
            if (value !== null && recordData.DateFinished !== null) return typeof value === "string" ? `'${value}'` : value
            return `NULL`
        }).join(", ")
            try {
                const query = `INSERT INTO [${table}] (${columns}) OUTPUT INSERTED.Id VALUES (${values})`
    ...
    async addRecordToPermissionTable( RoleId:any, AccessRightIds:any, CreatedBy:any, UpdatedBy:any ) {
        try {
            ...
            for (const AccessRightId of AccessRightIds) {
                ...
                const query = `INSERT INTO Permission (Code, RoleId, AccessRightId, CreatedBy, UpdatedBy, DateCreated, DateUpdated) OUTPUT INSERTED.Id
