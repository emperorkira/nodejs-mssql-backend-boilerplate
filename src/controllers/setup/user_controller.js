
import { GET, ADD, DELETE, UPDATE} from '../../models/index.js'
import { err_msg, success_msg, tbl, sql_query } from '../../shared/index.js';
import { user_schema } from '../../schemas/index.js';
import { getUserByUsername, generateCode, hashPassword, isPermission, isDefaultRecord, isFound } from '../../functions/index.js';
import { user_fields, user_action } from '../../type/index.js';
import sql from 'mssql';

    const { Int, NVarChar, DateTime, } = sql;
    
    export const current_user = async (req, res) => {
        try {
            if(!await isPermission(req.cookies.permissions, user_action.a1.Id)) return res.status(400).json({ message: err_msg.e00x24});
            if (!req.user) return res.status(400).json({ message: err_msg.e00x01});
            const user = await GET.record_by_id(req.user.user, tbl.t014);
            if (!user) return res.status(400).json({ message: err_msg.e00x05});
            return res.status(400).json({ data: user, message: success_msg.s00x00});
        } catch(error){
            return res.status(500).json({ message: err_msg.e00x02});
        }
    } // END HERE

    export const get_all = async (req, res) => {
        try {
            if(!await isPermission(req.cookies.permissions, user_action.a2.Id)) return res.status(400).json({ message: err_msg.e00x24});
            const users = await GET.all_record(tbl.t014);
            if (!users) return res.status(400).json({ message: err_msg.e00x23});
            return res.status(400).json({ data:users, message: success_msg.s00x00});
        } catch(error){
            return res.status(500).json({ message: err_msg.e00x02});
        }
    } // END HERE

    export const get = async (req, res) => {
        try {
            const { Id } = await req.params;
            if (!Id) return res.status(400).json({ message: err_msg.e00x07});
            if(!await isPermission(req.cookies.permissions, user_action.a3.Id)) return res.status(400).json({ message: err_msg.e00x24});
            const user = await GET.record_by_id(Id, tbl.t014);
            if (!user) return res.status(400).json({ message: err_msg.e00x05});
            return res.status(200).json({ data:user, message: success_msg.s00x00});
        } catch(error){
            return res.status(500).json({ message: err_msg.e00x02});
        }
    } // END HERE

    export const create = async (req, res) => {
        try {
            const body = await req.body;
            const code = await generateCode(tbl.t014);
            const current_user = req.user || 0;
            const current_date = new Date().toISOString();
            body.Code = code; body.CreatedBy = current_user; ; body.DateCreated = current_date;
            const { error } = user_schema.validate(body);
            if (error) return res.status(400).json({ message: err_msg.e00x01});
            if(!await isPermission(req.cookies.permissions, user_action.a4.Id)) return res.status(400).json({ message: err_msg.e00x24});
            if(await isFound('[dbo].[User]',['Username'],[NVarChar(255)],[body.Username])) return res.status(400).json({ message: err_msg.e00x06});
  
            const types = [
                NVarChar(50), NVarChar(255), NVarChar(255), NVarChar(50), NVarChar(50), 
                NVarChar(50), NVarChar(50), DateTime, NVarChar(255), NVarChar(255), 
                NVarChar(50), Int, Int, Int, Int, DateTime, Int, DateTime
            ];
            body.Password = await hashPassword(body.Password);
            const data = [
                code, body.Username, body.Password, body.Firstname, body.Middlename, body.Lastname,
                body.Gender, body.Birthdate, body.Address, body.ContactNumber, body.Image,
                body.DepartmentId, body.RoleId, body.isDeactivated, current_user, current_date, null, null
            ];
            if (!(await ADD.record(tbl.t014, user_fields, types, data))) return res.status(400).json({ message: err_msg.e00x03});
            return res.status(400).json({ message: success_msg.s00x02});
        } catch(error){
            return res.status(500).json({ message: err_msg.e00x02}); 
        }
    } // END HERE

    export const remove = async (req, res) => {
        try {
            const Id  = await req.params; 
            if (!Id) return res.status(400).json({ message: err_msg.e00x07});
            if(!await isPermission(req.cookies.permissions, user_action.a5.Id)) return res.status(400).json({ message: err_msg.e00x24});
            // valdiate if default or with transaction
            if(await isDefaultRecord(Id, tbl.t014) || await GET.record_by_fields(sql_query.q014x001, ['UserId'], [Int], [Id])) return res.status(400).json({ message: err_msg.e00x04});
            if (await DELETE.record_by_id(Id, tbl.t014)) return res.status(400).json({ message: err_msg.e00x03});
            return res.status(400).json({ message: success_msg.s00x04});
        } catch(error){
            return res.status(500).json({ message: err_msg.e00x02});
        }
    } // END HERE

    export const update = async (req, res) => {
        try {
            const Id = await req.params;
            if (!Id) return res.status(400).json({ message: err_msg.e00x07});
            const body = await req.body;
            const { error } = user_schema.validate(body);
            if (error) return res.status(400).json({ message: err_msg.e00x01});
            if(!await isPermission(req.cookies.permissions, user_action.a6.Id)) return res.status(400).json({ message: err_msg.e00x24});
            if (await isDefaultRecord(Id,tbl.t014)) return res.status(400).json({ message: err_msg.e00x10});
            const fieldsToRemove = ['Code', 'Password', 'CreatedBy', 'DateCreated'];
            const updated_user_fields = user_fields.filter(field => !fieldsToRemove.includes(field));
            const update_user = req.user || 0;
            const update_date = new Date().toISOString();
            const types = [
                NVarChar(255), NVarChar(50), NVarChar(50), 
                NVarChar(50), NVarChar(50), Date, NVarChar(255), NVarChar(255), 
                NVarChar(50), Int, Int, Int, Int, DateTime
            ];
            const data = [
                body.Username, body.Firstname, body.Middlename, body.Lastname,
                body.Gender, body.Birthdate, body.Address, body.ContactNumber, body.Image,
                body.DepartmentId, body.RoleId, body.isDeactivated, update_user, update_date
            ];
            if (!await UPDATE.record(Id, updated_user_fields, types, data))  return res.status(400).json({ message: err_msg.e00x03 });
            return res.status(400).json({ message: success_msg.s00x04});
        } catch(error){
            return res.status(500).json({ message: err_msg.e00x02});
        }
    } // END HERE

    export const change_pass = async (req, res) => {
        try {
            const Id = await req.params;
            if (!Id) return res.status(400).json({ message: err_msg.e00x07});
            if(!await isPermission(req.cookies.permissions, user_action.a7.Id)) return res.status(400).json({ message: err_msg.e00x24});
            const { Password } = await req.body;
            const { error } = user_schema.validate({Password});
            if (error) return res.status(400).json({ message: err_msg.e00x01});
            const hashed = await hashPassword(Password);
            if (!await UPDATE.record(Id, ['Password'], [NVarChar(255)], [hashed]))  return res.status(400).json({ message: err_msg.e00x03 });
            return res.status(400).json({ message: success_msg.s00x04});
        } catch(error){
            return res.status(500).json({ message: err_msg.e00x02});
        }
    } // END HERE
    
    export const user_controller_test = async (req, res) => {
        try {
            const USER_PERMISSION = (req.cookies.permissions);
            if(!await isPermission(USER_PERMISSION, 58)) return res.status(400).json({ message: err_msg.e00x24})
            return res.status(400).json({ permission: USER_PERMISSION, message: success_msg.s00x04});
        } catch(error){
            return res.status(500).json({ message: err_msg.e00x02});
        }
    } // END HERE